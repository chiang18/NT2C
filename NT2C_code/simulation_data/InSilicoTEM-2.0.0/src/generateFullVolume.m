...
%     save([pwd filesep 'Particles' filesep TransName], 'transl'); 
    ...
    for ss=1:numPart
        ...
        nx=str2double(tokens{1}{2});
        ny=str2double(tokens{1}{3});
        nz=str2double(tokens{1}{4});
        ...
        %xy0 = [N/2+round(xt(ss)) N/2+round(yt(ss))]+1;
        %disp(xt)
        xy0 = [N/2+xt(ss) N/2+yt(ss)]+1; %N1024    粒子中心
        xy1 = xy0 + [nx ny] - [1,1]; %[nx ny颗粒的大小 xy1右下座標 
        disp(xy0)
        disp(xy1)
        disp(floor(nx/2))
        disp(floor(ny/2))
        if xy0(1)-floor(nx/2) < 0
            a = abs(xy0(1)-floor(nx/2)) + 1;
            x = xy0(1)-floor(nx/2)+ a : xy1(1)-floor(nx/2) + a;
        elseif xy1(1)-floor(nx/2) > 1024
            aa = abs(xy1(1)-floor(nx/2))-1024 ;
            x = xy0(1)-floor(nx/2)- aa : xy1(1)-floor(nx/2) - aa;
        else
            x = xy0(1)-floor(nx/2) : xy1(1)-floor(nx/2);
        end

        if xy0(2)-floor(ny/2) < 0
            b = abs(xy0(2)-floor(ny/2)) + 1;
            y = xy0(2)-floor(ny/2)+ b : xy1(2)-floor(ny/2) + b;
        else
            y = xy0(2)-floor(ny/2) : xy1(2)-floor(ny/2);
        end
        %x   = xy0(1)-floor(nx/2):xy1(1)-floor(nx/2);
        %y   = xy0(2)-floor(ny/2):xy1(2)-floor(ny/2);
        %x   = xy0(1)-floor(nx/2):xy0(1)+floor(nx/2);
        %y   = xy0(2)-floor(ny/2):xy0(2)+floor(ny/2);
        z   = round(szz/2-nz/2)+zt(ss) : round(szz/2+nz/2)-1+zt(ss); 
        %if xy0(1) - floor(nx/2) < 1 || xy0(2) - floor(ny/2) < 1 || xy1(1)-floor(nx/2) > N || xy1(2)-floor(ny/2) > N
           % continue; % 跳过该颗粒的处理
        %end
        %disp('x values:');
        %disp(x);
...
elseif strcmp(params2.spec.source, 'map')
    ...
    for ss=1:numpart
        ...
        xy1 = xy0 + [nxr nyr] - [1,1];
        x   = abs(xy0(1)-floor(nxr/2)):xy1(1) - floor(nxr/2);
        y   = abs(xy0(2)-floor(nyr/2)):xy1(2) - floor(nyr/2);
        z   = round(szz/2-nzr/2) + zt(ss) : round(szz/2+nzr/2) - 1 + zt(ss); 
        volstruct(abs(x),abs(y),z) = atompot4rot;
    end
    VolPot = dip_image(permute(volstruct,[2 1 3]),'scomplex');  
