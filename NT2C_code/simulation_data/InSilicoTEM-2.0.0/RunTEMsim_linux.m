...
%                                                                         %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %  
%run("/home/m112040012/NT2C/DIPimage 2.9/dipstart.m")
addpath("/home/m112040012/InsilicoTEM/dipimage_2.9_lin64/dip/Linuxa64/lib/")
addpath("/home/m112040012/InsilicoTEM/dipimage_2.9_lin64/dip/common/dipimage")
dip_initialise;
...
% Parameters
mg = 1000;               % Number of micrographs to generate
pp = 0;                 % Phase plate (0 = no; 1 = yes)
df_range= [700 2600];    % Defocus range [nm]
mb_series = [0];      % Motion blur series (If multiple MB, enter them as a vector)
cf_series = [0];       % Correction factor  (If multiple CF, enter them as a vector)
dose      = [30];       % Electron dose to the specimen [e-/A2]
pix = 1024;             % Number of pixels 
pixsize = 1.16;          % Pixel size [A]
mindist = 210/pixsize;   % Minimum distance between particles divided by pixsize, 
...
tic
 ...
 rpdb = pdbread('./PDBs/5lzf-pdb-bundle1.pdb'); % Select PDB file to read
toc
disp(' ')               
                
delete(gcp('nocreate')); % 結束現有的並行會話
parpool('local', 16); % 啟動新的並行池，並限制為最多 4 個核心
    ...
    for cf = 1 : length(cf_series)
        for d = 1:length(dose)
            for mb = 1 : length (mb_series)    
                try
                    %count = count + 1; 
                    local_count = local_count + 16+1; % 更新本地计数器
                    ou = tic;
                    ...
                    disp( "-----------------------   Progress:   "+local_count+" / "+tot+" Micrographs   -----------------------")
                    disp(' ')
                    ...
                    ImageOut = out.series; %.noiseless_series
                    ImageOut2 = out.noiseless_series; %.noiseless_series
                    delete ./Raw/Particles/*.raw;  % delete particle positions for the generated micrographs (for memory)
                    
                    
                    % Write micrograph to .MRC file
                    s1 = "s3b1MicrographNr_5lzf"+ (micro) +"_size"+pix+"x"+pix+"_pixsize"+pixsize*100+"_partnr"+particles+"_dose"+dose(d)+"_cf"+cf_series(cf)+"_mb"+mb_series(cf)+"_df"+round(defocus)+"_PP"+pp+".mrc";
                    WriteMRC(double(ImageOut),pixsize,[dir_noise filesep char(s1)]);
                    WriteMRC(double(ImageOut2),pixsize,[dir_clean filesep char(s1)]);
                    disp(' ')
