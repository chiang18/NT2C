...
for input_file in input_list:
    ...
    denoised_img = np.array(Image.open(os.path.join(denoised_dir,input_file)))
    denoised_img=((denoised_img-np.min(denoised_img))/(np.max(denoised_img)-np.min(denoised_img))*255).astype(np.uint8)

    # raw_img=mrcfile.open(os.path.join(raw_dir,input_file),permissive=True)
    # raw_img=raw_img.data
    raw_img = np.array(Image.open(os.path.join(raw_dir,input_file)))
    raw_img=((raw_img-np.min(raw_img))/(np.max(raw_img)-np.min(raw_img))*255).astype(np.uint8)
    ...
    for x in range(0,w,step):
        for y in range(0,h,step):
            ...
            if noise:
                ...
                cv2.imwrite(noise_patch_path[:-4]+'_'+str(noise_patch_n)+'.png', raw_arr);
                #draw noise
    ...
    arr=(denoised_img * 255).astype(np.uint8)
    cv2.imwrite(draw_noise_path, denoised_img);
