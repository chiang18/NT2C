...
def make_paired_images_datasets(dir_a, dir_b, crop, random=np.random, holdout=0.1, preload=False, cutoff=0):
    ...
    A = []
    B = []
    for path in glob.glob(dir_a + os.sep + '*.jpg'):
    ...
    dataset_train = dn.PairedImages(A_train, B_train, crop=crop, xform=False, preload=preload, cutoff=cutoff)
...
def make_images_datasets(dir_a, dir_b, crop, random=np.random, holdout=0.1, cutoff=0):
    ...
    dataset_train = dn.NoiseImages(path_train, crop=crop, xform=False, cutoff=cutoff)
...
def check_dataset_for_nan(dataset):
    for x1, x2 in dataset:
        x1 = torch.from_numpy(x1)  # 將 numpy.ndarray 轉換為 torch.Tensor
        x2 = torch.from_numpy(x2)  # 將 numpy.ndarray 轉換為 torch.Tensor

        if torch.isnan(x1).any():
            print("Dataset contains NaN values in x1.")
            return
        elif torch.isnan(x2).any():
            print("Dataset contains NaN values in x2.")
            return
...
#             print('bb')
#             check_dataset_for_nan(dataset_train)
#             check_dataset_for_nan(dataset_val)
