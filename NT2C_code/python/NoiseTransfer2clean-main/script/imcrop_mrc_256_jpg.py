    ...
    for x in range(start,sizex,step):
        for y in range(start,sizey,step):
            ...
            if oldx !=startx or oldy!=starty:
                ...
                arr=norm(arr)
                if np.all(arr == 0) or np.isnan(arr).any():
                    arr[:, :]=120.30241935
                fname=f0+'.'+str(startx)+'.'+str(starty)+'.'+str(size)+'.jpg';
                ...
                #cv2.imwrite(fout, arr*255, [cv2.IMWRITE_JPEG_QUALITY, 75]); 原使用
                #print(arr)
                # name=dout.split('/');
                # name=name[-1];
                # line=fname+'\n';
...
#flist=open(flist,'w');
list = os.listdir(din)[:]
#list = os.listdir("/home/m112040012/vscode/data/10077/pre_wgan_reweight_nob_jpg/sb1_210512 pos 1000 1-3_1.jpg")
#mrcs=mrcfile.open()
