...
#     Xi=np.array([i for j in clean for i in j],dtype=np.uint8)
#     Yi=np.array([i for j in noise for i in j],dtype=np.uint8)
#     Zi=np.array([i for j in noisy for i in j],dtype=np.uint8)
...
def add_noise(clean_img):
    ...
    file_name, file_ext = os.path.splitext(noise_path)
    if file_ext=='.jpg':
        noise=cv2.imread(noise_path, cv2.IMREAD_GRAYSCALE)#jpg 
    else: 
        noise=mrcfile.open(noise_path,permissive=True) #mrc
        noise=noise.data
...
#     i=i+1
    ...
    noisy_gen=k1*clean+k2*(noise-np.mean(noise))+(noise-np.mean(noise))
    if np.all(noisy_gen == 0) or np.isnan(noisy_gen).any():
        noisy_gen=noise+0.5*clean
    #noisy_gen=k1*clean+k2*noise+b+noise

    #noisy_gen=clean+noise
    
    noisy_gen=((noisy_gen-np.min(noisy_gen))/(np.max(noisy_gen)-np.min(noisy_gen)))*255
    
    #print('noisy_gen',noisy_gen.max(),noisy_gen.min())
    #noisy_gen = noisy_gen.reshape((640,640))
    #noisy_gen = noisy_gen.reshape((320,320))
    #noisy_gen = noisy_gen.reshape((256,256))
    cv2.imwrite(noisy_gen_dir+clean_img,noisy_gen)
    #fit_noisy_out.set_data(noisy_gen.astype(np.float32))
