...
output_zip = os.path.join(noise_dir, 'noise_patches.zip')
draw_noise_zip = os.path.join(draw_dir, 'draw_noise.zip')
...
with zipfile.ZipFile(draw_noise_zip, 'w', zipfile.ZIP_DEFLATED) as draw_zipf, zipfile.ZipFile(output_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
  input_list=os.listdir(denoised_dir)[:1]
  print(input_list)
  for input_file in input_list:
      ...
      for x in range(0,w,step):
          for y in range(0,h,step):
              ...
              if noise:
                  ...
                  with  mrcfile.new(noise_patch_path[:-4]+'_'+str(noise_patch_n)+'.mrc',overwrite=True) as mrc_out:
                      mrc_out.set_data(raw_arr.astype(np.float32))
                      mrc_out.close()
                      zipf.write(noise_patch_path[:-4]+'_'+str(noise_patch_n)+'.mrc')
                      os.remove(noise_patch_path[:-4]+'_'+str(noise_patch_n)+'.mrc')
      ...
      with  mrcfile.new(input_file, overwrite=True) as mrc_out:
          mrc_out.set_data(denoised_img.astype(np.float32))
          mrc_out.close()
