...
def process_micrograph(path, models, lowpass, cutoff, gaus, inv_gaus, deconvolve, deconv_patch
                              , patch_size, padding, normalize, use_cuda, suffix, format_):
            ...
            mic = denoise_image(mic, models, lowpass=lowpass, cutoff=cutoff, gaus=gaus
                                ...
                                , patch_size=patch_size, padding=padding, normalize=normalize
                                , use_cuda=use_cuda
                                )
...
#         output_zip = os.path.join(args.output, '10077_pre_topaztest.zip')
#         with zipfile.ZipFile(output_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:

        for path in args.micrographs: #[-100:]
              name,_ = os.path.splitext(os.path.basename(path))
              mic = np.array(load_image(path), copy=False).astype(np.float32)
            ...
            #               mrc_path=name+'.'+format_
            #               #print(mrc_path)
            #               mrc_out = mrcfile.new(mrc_path, overwrite=True)
            #               mrc_out.set_data(mic.astype(np.float32))
            #               mrc_out.close()
            #               zipf.write(mrc_path)
            #               os.remove(mrc_path)
